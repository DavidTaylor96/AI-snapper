...
impl ScreenshotCapture {
    ...
    pub async fn capture(&self) -> Result<Vec<u8>> {
        ...
        tracing::info!(
            "Capturing screenshot from screen with display info: {:?}",
            screen.display_info
        );
        ...
        tracing::info!(
            "Raw image data - Width: {}, Height: {}, Buffer length: {}",
            width,
            height,
            buffer.len()
        );
    ...
    pub fn optimize_image(&self, image: &screenshots::Image) -> Result<Vec<u8>> {
        ...
        if bytes_per_pixel < 1.0 {
            ...
            if let Ok(decoded) =
                image::load_from_memory_with_format(rgba_data, image::ImageFormat::Png)
            {
            ...
            if let Ok(decoded) =
                image::load_from_memory_with_format(rgba_data, image::ImageFormat::Jpeg)
            {
        ...
        let dynamic_img =
            if rgba_data.len() == expected_rgba {
                ...
                let img =
                    image::ImageBuffer::from_raw(width, height, rgba_vec).ok_or_else(|| {
                        anyhow::anyhow!("Failed to create RGBA image buffer from RGB")
                    })?;
                image::DynamicImage::ImageRgba8(img)
            } else {
